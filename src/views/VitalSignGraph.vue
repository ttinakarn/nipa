  ...
  mounted() {
    ...
    axios
      ...
      .then(function(response) {
        ...
        instance.lastesttemp = instance.temp.slice(
          instance.temp.length - 8,
          instance.temp.length
        );
        instance.lastesttempdate = instance.tempdate.slice(
          instance.tempdate.length - 8,
          instance.tempdate.length
        );
        for (var i = 0; i < response.data.data.length; i++) {
        ...
        instance.lastestpulse = instance.pulse.slice(
          instance.pulse.length - 8,
          instance.pulse.length
        );
        instance.lastestpulsedate = instance.pulsedate.slice(
          instance.pulsedate.length - 8,
          instance.pulsedate.length
        );
      });
  },
  methods: {
    ...
    onExport() {
      this.VS = JSON.parse(JSON.stringify(this.vitalsigns))
      for (var i = 0; i < this.VS.length; i++) {
        this.VS[i].admitdate = moment(
          this.VS[i].admitdate
        ).format("ll");
        if (this.VS[i].dischargedate != null) {
          this.VS[i].dischargedate = moment(
            this.VS[i].dischargedate
          ).format("ll");
        }
        this.VS[i].date = moment(this.VS[i].date).format("lll");
      }
      const dataWS = XLSX.utils.json_to_sheet(this.VS);
